...
\begin{figure*}[!bth]
  ...
  \caption{\small
    \textbf{Example of Different Layouts and Tiling Schemes:}
      ...
      The figures below show how the different combinatons might partition a
        grid into four tiles each containing \((ni^2/4)(nk)\) elements (one 
        tile is shown).
      With the tile-\(ij\) scheme we get \(ni/2 \times ni/2 \times nk\) tiles,
        and with the tile-\(i\) scheme we get \(ni \times ni/4 \times nk\) tiles.
      Different colors indicate different contiguous regions within the tile.
      Regions with similar colors are closer to each other in memory.
  }
  ...
  \begin{minipage}{0.49\textwidth}
    ...
    \subfloat[
      ...
      \((ni/2)(nk)\) contiguous \(i\)-rows each containing \(ni/2\) elements.
      This layout and tiling combination exhibits the worst contiguity out 
  ...
  \begin{minipage}{0.49\textwidth}
    ...
    \subfloat[
      ...
      \(nk\) contiguous \(ij\)-planes, each containing \(ni^2/4\) elements.
  ...
  \begin{minipage}{0.49\textwidth}
    ...
    \subfloat[
      ...
      \((ni/2)(nk)\) contiguous \(i\)-rows, each containing \(ni/2\) elements.
      This layout and tiling combination exhibits the second-best contiguity 
        out of the four different options.
    ]{        
  ...
  \begin{minipage}{0.49\textwidth}
    ...
    \subfloat[
      ...
      One contiguous region containing \((ni^2/4)(nk)\) elements.
...
For example, suppose we partition an \(ijk\)-layout grid into four tiles each
  containing \((ni^2/4)(nk)\) elements, with the tile-\(ij\) scheme producing
...
The tile-\(ij\) tiles would be the same as before: \(ni/2 \times ni/2 \times nk\)
  tiles consisting of \((ni/2)(nk)\) contiguous \(i\)-rows of only \(ni/2\)
...
We developed SSTA to replace the MKL-based vertical column
  solver in our production climate application, initially because
  the matrix size was not a multiple of the KNL vector length, but
  also because we believed better vectorization and data movement
  would be achieved by solving multiple vertical columns simultaneously.
...
\item The MKL baseline solver would be very insensitive to total tile size,
  ...
  exploit vector operations and data locality like a multi-column solver would.
