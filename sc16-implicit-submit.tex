...
We evaluate the performance of our approach and present an optimized
  tridiagonal matrix solver that uses only OpenMP and compiler directives,
  tiling, and memory layouts to optimize vectorization and memory bandwidth.
Performance is evaluated on Intel microarchitectures with different cache,
  vectorization, and threading features: Intel Ivy Bridge, Haswell, and Knight's
  Landing.
Finally, we show that our solver improves on existing approaches and achieves
  \(\sim 90\%\) of STREAM Triad bandwidth on all three target microarchitectures.
\end{abstract}
...
%  \caption{
%    \textbf{Tile-\(ij\) Scheme with the \(ijk\)-Layout:}
%    Suppose we partition an \textbf{\(ijk\)-layout} grid into tiles containing
%      \(nk \times ni^2/4\) elements, producing \(ni/2 \times ni/2 \times nk\)
%      tiles with the \textbf{tile-\(ij\)} scheme.
%    Each tile consists of \(nk \times ni/2\) contiguous \(i\)-lines (colored
%      below), each containing \(ni/2\) elements.
%    This layout and tiling combination exhibits the least contiguity out of the
%      four different options.
...
%    \textbf{Tile-\(j\) Scheme with the \(ijk\)-Layout:}
%    Suppose we partition an \textbf{\(ijk\)-layout} grid into tiles containing
%      \(nk \times ni^2/4\) elements, producing \(ni \times ni/4 \times nk\)
%      tiles with the \textbf{tile-\(j\)} scheme.
%    Each tile consists of \(nk\) contiguous \(ij\)-planes (colored
%      below), each containing \(ni^2/4\) elements.
%    This layout and tiling combination exhibits the best contiguity for the 
%      \(ijk\)-layout.
...
%  \caption{
%    \textbf{Tile-\(ij\) Scheme with the \(ikj\)-Layout:}
%    Suppose we partition an \textbf{\(ikj\)-layout} grid into tiles containing
%      \(nk \times ni^2/4\) elements, producing \(ni/2 \times ni/2 \times nk\)
%      tiles with the \textbf{tile-\(ij\)} scheme.
%    Each tile consists of \(ni/2\) contiguous \(ik\)-planes (colored
%      below), each containing \(nk \times ni/2\) elements.
...
%      All measurements are averages of a statistically significant number of
%        samples; 95\% confidence intervals are shown, although some are not
%        visible due to their small magnitude.
    }
...
\begin{table*}[t]
  ...
  \caption{
    ...
    Optimal tile sizes, effective bandwidth, percentage of STREAM Triad performance
      attained and speedup over the MKL baseline solver are given below for all
      SSTA experiments.
    ...
    The optimal tile size for the \(ikj\)-layout, which exhibits greater data
      contiguity, is smaller and will fit in the L2 on all platforms.
    \(\sim\) 90\% of STREAM Triad effective bandwidth is obtained on all
      platforms.
    On the Xeon platforms, the SSTA solver is a \(\sim 2x\) speedup over the 
      MKL baseline; on Knight's Landing it is a \(\sim 12x\) speedup.  
    All measurements are averages of a statistically significant number of
