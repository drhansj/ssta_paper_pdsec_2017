...
wish to solve $A * u^{s+1} = u^{s}$ for $u^{s}$:
...
or the tridiagonal matrix algorithm (TDMA)~\cite{}, and it is \(O(n)\) in time,
...
The Thomas algorithm consists of two passes. First, a forward pass is performed
to eliminate the $a_i$ elements~\cite{}:
...
% TODO: Describe cached-divide algorithm
...
(e.g. \(z\)) column in a \(nx\)x\(ny\)x\(nz\) Cartesian grid (Figure~\ref{1}).
...
separately, independent of the other solves. An \(nz\)x\(nz\) tridiagonal
...
not possible - each iteration of the solve is dependent on previous iterations.

The other approach is to simultaneously solve multiple columns. A block
\(nz\)x\(nz\) tridiagonal matrix is constructed for the whole grid; each block
contained within the matrix is an \(nx\)x\(ny\) horizontal plane (e.g. the
block matrix is a 4D space). The entire 3D Cartesian grid is viewed as an
\(nz\) block vector of \(nx\)x\(ny\) planes. The Thomas algorithm is then
...
algorithm can be vectorized across the \(nx\)x\(ny\) horizontal plane that it
...
remove these temporary buffer allocations. We observed a noticable performance
...
decided to switch back to an \(ijk\) layout.
