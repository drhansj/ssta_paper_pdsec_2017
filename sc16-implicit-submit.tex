...
experiments in this paper. \fxnote{Rename the Github repo from tridiagonal\_solve\_benchmarks to tridiagonal\_solve\_benchmark}

% \fxnote{Paragraph describing test problem and grid set up.}
The benchmark suite contains a multi-dimensional array abstraction, generic
solver components and a driver. The benchmark solves the following
one-dimensional diffusion equation in each vertical column of a 3D cartesian
grid using the implicit Backward Time, Centered Space (BTCS) finite difference
method:
...
arbitrary. In the tile-j scheme that we use, the extent of i controls the
...
\fxnote{Paragraph about different variants.}
...
\fxnote{Paragraph about profiling.}
